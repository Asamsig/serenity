...
import serenity.users.domain
import serenity.users.domain._

import scala.concurrent.Future

@Singleton
class UserRepository @Inject()(val dbConfigProvider: DatabaseConfigProvider) extends Tables {
  ...
  private def insertUserAction(uid: UserId, u: User) = {
    val row: UserRow = (uid, u.firstName, u.lastName, u.phone, u.address, u.createdDate, UtcDateTime.nowUTC())
    usersTable += row
  }
