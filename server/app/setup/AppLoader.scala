...
class AppLoader extends GuiceApplicationLoader {
  protected override def overrides(
    ...
    super.overrides(context) :+ (inject
      .bind[Router]
      .toProvider[ScalaRoutesProvider]: GuiceableModule)
  }
}

@Singleton
class ScalaRoutesProvider @Inject()(router: AppRoutes, httpConfig: HttpConfiguration)
