...
object DomainProtobufFormats {
  ...
  implicit def protoUuidToJavaUuid(id: Option[PUUID]): UUID = id match {
    ...
    case None =>
      throw new IllegalStateException("id is required and isn't allowed to be None")
  }
  ...
  implicit def fromTimestamp(ts: Option[Timestamp]): LocalDate =
    LocalDateTime
      .ofInstant(
        Instant.ofEpochSecond(ts.get.seconds.toLong, ts.get.nanos.toLong),
        ZoneOffset.UTC
      )
      .toLocalDate
  ...
  implicit val hospesUserImportPBP = new ProtobufFormat[HospesUserImportEvt] {
    ...
    override def write(e: HospesUserImportEvt): Message =
      HospesUserImportMessage.toJavaProto(
        HospesUserImportMessage(
          e.id.underlying,
          e.originId,
  ...
  implicit val userRegisteredPBP = new ProtobufFormat[UserUpdatedEvt] {
    ...
    override def write(e: UserUpdatedEvt): Message =
      UserUpdatedMessage.toJavaProto(
        UserUpdatedMessage(
          e.id.underlying,
          ...
          m.action match {
            ...
            case e @ _ =>
              throw new IllegalArgumentException(
                s"Unknown enum for ActionEnum. Value: $e"
              )
          },
          m.issuer match {
            ...
            case e @ _ =>
              throw new IllegalArgumentException(
                s"Unknown enum for IssuerEnum. Value: $e"
              )
          },
          m.eventbriteInformation
            .map(ei => EventbriteMeta(ei.attendeeId, ei.eventId, ei.orderId)),
          m.meta
        )
    }

    override def write(e: MembershipUpdateEvt): Message =
      MembershipUpdateMessage.toJavaProto(
        MembershipUpdateMessage(
          e.from,
          e.from.plusYears(1).minusDays(1),
          e.eventbirteMeta
            .map(em => EventbriteInformation(em.attendeeId, em.eventId, em.orderId)),
          e.issuer match {
