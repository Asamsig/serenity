...
package object graphql {
  ...
  case object LocalDateTimeCoercionViolation extends ValueCoercionViolation("DateTime with time zone expected")
  ...
  implicit val UUIDType: ScalarType[UUID] = ScalarType[UUID]("UUID",
    ...
    coerceOutput = (value: UUID, capabilities: Set[MarshallerCapability]) =>
      value.toString,
    coerceUserInput = {
  ...
  implicit val LocalDateType: ScalarType[LocalDate] = ScalarType[LocalDate]("LocalDate",
    description = Some(
      "The `LocalDate` scalar type represents a date"),
    coerceOutput = (value: LocalDate, capabilities: Set[MarshallerCapability]) =>
      value.toString,
    coerceUserInput = {
